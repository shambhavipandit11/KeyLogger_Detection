...
SUSPICIOUS_KEYWORDS = [
    "keylogger", "klog", "logger", "spy", "hook", "pynput", "keyboard_monitor", "real_poc", 
]
...
def log_incident(message):
    """Helper function to save scan history permanently"""
    log_file_path = os.path.join(os.path.dirname(__file__), "scan_history.txt")
...
def scan_processes():
    ...
    print("="*50)
    log_incident("INFO: Live Detection Scan Launched.") # <--- ADD THIS LINE
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if is_malicious:
                detected_threats += 1
                log_incident(f"ALERT: Suspicious process identified -> Name: {name} | PID: {pid} | Reason: {reason}") # <--- ADD THIS LINE
                ...
                if choice == 'y':
                    try:
                       ...
                       p.terminate()
                       log_incident(f"REMEDIATION: Forcefully terminated suspicious PID {pid}.") # <--- ADD THIS LINE
    ...
    if detected_threats == 0:
        log_incident("INFO: Scan Completed. System clean.") # <--- ADD THIS LINE
        print("[+] Scan Completed. System appears clean. No active keyloggers found.")
    else:
        log_incident(f"INFO: Scan Completed. Action taken on {detected_threats} threat(s).") # <--- ADD THIS LINE
